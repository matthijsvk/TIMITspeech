...
def processDataset(FRAC_TRAINING, data_source_path, logger=None):
    ...
    X_all, y_all, valid_frames_all = preprocessWavs.preprocess_dataset(source_path=data_source_path, nbMFCCs=nbMFCCs,
                                                                       logger=logger, debug=debug_size)
...
def processDatasetSplit(train_source_path, test_source_path, logger=None):
    ...
    X_training, y_training, valid_frames_training = preprocessWavs.preprocess_dataset(source_path=train_source_path,
                                                                                      logger=logger,
                                                                                      nbMFCCs=nbMFCCs, debug=debug_size)
    logger.info('  Test data: %s', test_source_path)
    X_test, y_test, valid_frames_test = preprocessWavs.preprocess_dataset(source_path=test_source_path, logger=logger,
                                                                          nbMFCCs=nbMFCCs, debug=debug_size)
    return X_training, y_training, valid_frames_training, X_test, y_test, valid_frames_test


if dataPreSplit:
    X_training, y_training, valid_frames_training, X_test, y_test, valid_frames_test = \
