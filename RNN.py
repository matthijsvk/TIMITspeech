            ...
            if justTest:
                for test_dataset in test_datasets[dataset]:
                    for batch_size in batch_sizes:
                        try:
                            network, loadParamsSuccess, model_save, batch_size, fh = setupNetwork(dataset, test_dataset,
                                                                                                  N_HIDDEN_LIST,
                                                                                                  batch_size,
                                                                                                  ROUND_PARAMS)
                            if withNoise:
                                for noiseType in noiseTypes:
                                    for ratio_dB in ratio_dBs:
                                        ...
                                        trainNetwork(network=network, loadParamsSuccess=loadParamsSuccess,
                                                     model_save=model_save, batch_size=batch_size,
...
def setupNetwork(dataset, test_dataset, N_HIDDEN_LIST, batch_size, ROUND_PARAMS=False):
    ...
    success = RNN_network.load_model(model_load, roundParams=ROUND_PARAMS)
...
def trainNetwork(network, loadParamsSuccess, model_save, batch_size, datasetFiles, withNoise=False, noiseType='white',
                 ratio_dB=0, fh=None):

    LR = LR_start
    if loadParamsSuccess == 0: LR = LR_start / 10.0
